...
# Essential for outputting predicted images
import os.path
    ...
    for index in range(total_epoch):
        ...
        with torch.no_grad():  # operations inside don't track history
            ...
            for k in range(test_batch):
                # test_data, test_label, test_depth = test_dataset.next()        # 旧版本 pytorch 语法
...
def multi_task_trainer_AdaCorr(train_loader, test_loader, multi_task_model, device, optimize, scheduler, opt, total_epoch=200):
    train_batch = len(train_loader)
    test_batch = len(test_loader)
    # T = opt.temp        # '--temp', default=2.0        # The DWA algorithm requires the use of
    avg_cost = np.zeros([total_epoch, 12], dtype=np.float32)  # total_epoch=200
    # lambda_weight = np.ones([2, total_epoch])        # The DWA algorithm requires the use of
    for index in range(total_epoch):
        cost = np.zeros(12, dtype=np.float32)
